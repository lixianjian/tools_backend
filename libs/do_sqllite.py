...
class SqlLite:
    ...
    def find(self, name, spec=None, fields=None, other=None, req_str=None):
        #select top n * from temp_table order by id desc
        if not objIsEmpty(fields):
            fields_str = string.join(fields, ',')
        elif not objIsEmpty(req_str):
            fields_str = req_str   
        else:
            fields_str = '*'
