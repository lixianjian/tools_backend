...
def get_last_hour():
    ...
    weehours  = datetime(year=today.year, month=today.month, day=today.day, hour=today.hour)
            ...
            for info in infos:
                ...
                if read_other:
                    ...
                    if other_v is not None:
                        d1  = get_from_fname(path=other_path,fname=other_v,fields=other_fields)
...
def get_data_from_url(url, ret_field='data', is_compress=False,is_json=True, method='GET',values=None):
    """ 通过访问url获取数据 """
    for i in range(1,4):
        htmlmsg = visit_url(url, download=False,is_compress=is_compress,is_json=is_json, method=method,values=values)
...
def get_log_files(src_path, date_time, hour='',run_hour=True,run_hours=None):
    ...
    #指定某一个小时的日志文件
    if run_hour:                    
        fname = '%s/%s/%s/game_%s_%s.log'%(src_path, date_time, hour, date_time, hour)
        if os.path.exists(fname):
            fnames.append(fname)
        else:
...
def judge_internalip(ip):
    ...
    if '127.0.0.1' == ip or '10.23.1.' in ip:
        return True
    msg = visit_url(LAN_IP_URL, download=False, is_json=False, is_compress=False)
    print 'msg: ', msg
    if msg['status'] == 0:
        company_ip  = msg['data']
        print 'ip:',ip
...
#        print help(timeout)
    ...
    if 0:
        ...
        for ip in ips:
            data    = ip2area(ip)
            datas.append('%s\t%s\t%s\t%s'%(data['country'],data['area'],data['region'],data['city']))
        for data in datas:
            print data
#        for k,v in ip2area(ips[0]).items():
#            print k,v
        #print get_from_name('D:/check_data/db/sea/sea_11/sea_role',k='name',ps=u'大兵爱流汗',fields=['name','roleid'])
...
#        sql.disconnect()
    ...
    if 0:
        ...
        d2={'c1':{'count':3,'roles':['1','4']},'c2':{'count':3,'roles':['1','3']}}
        merge_dict(d1,d2)
        print d1
