...
from libs.config import glocale, DEBUG, PER_PAGE, ASSETS_URL, \
    PARAM_DEBUG_PATH, BASE_AUTO_ADD_FIELD, AUTO_ADD_FIELD2, \
    PROGRAM_PATH, BS_DB_NAME
    ...
    for sheet in Sheet.objects.filter(program=program, locale=locale):
        ...
        if sheets.has_key(sys_markid):
            if sheets[sys_markid]['sub_sheets'].has_key(sys_markid2):
                ...
                sheets[sys_markid]['sub_sheets'][sys_markid2] = {
...
def some_view(data, output_name='excel_data'):
    ...
    response[
        'Content-Disposition'] = 'attachment; filename=%s.csv' % output_name
...
def alter_password(request):
    ...
    if request.method == 'GET':
        ...
        # return
        # render_to_response(INDEX_PAGE,content,context_instance=RequestContext(request))
    elif request.method == 'POST':
        msg = alter(request)
        if msg.get('status', 1) == 0:
            content.update({'tips': msg.get(
                'tips', glocale.LN_CHOOSE_FUNC_IN_LEFT_FRAME), 'html_path': 'login/alter.html'})
        else:
            content.update(
                {'tips': msg.get('tips', glocale.LN_FAIL), 'html_path': 'login/alter.html'})
    else:
        return HttpResponseServerError()
...
def choose_param(request, tips=''):
    ...
    if public.objIsFull(pid) and public.objIsFull(localeid):
        ...
        try:
            ...
            locale = None
        try:
            prog_loc_user = ProgramLocaleUser.objects.get(
                program=program.id, locale=locale.id, user=request.user.id)
        except ProgramLocaleUser.DoesNotExist:
            prog_loc_user = None
        content = {'program': program, 'locale': locale, 'sheets': get_sheets(
            program=program.id, locale=locale.id), 'prog_loc_user': prog_loc_user}
        content.update(csrf(request))
        return render_to_response(INDEX_PAGE, content, context_instance=RequestContext(request))
    else:
        ...
        if public.objIsEmpty(objs):
            content = {
                'tips': tips + u'抱歉，目前没有项目，请先添加', 'form_action': 'choose_param'}
        else:
...
def param_main_response(request, pid=None, localeid=None, sheetid=None):
    ...
    prog_loc_user = get_contact(
        ProgramLocaleUser, program=program.id, locale=locale.id, user=request.user.id)
    info = {'program': program, 'locale': locale, 'sheets': get_sheets(
        program=program.id, locale=locale.id), 'sheet': sheet, 'prog_loc_user': prog_loc_user}
#    print info['sheets']
...
#        self.manytomanys= {}
    ...
    def alter_password(self):
        ...
        if self.request.method == 'GET':
            ...
            if msg.get('status', 1) == 0:
                self.content.update(
                    {'tips': msg.get('tips', glocale.LN_CHOOSE_FUNC_IN_LEFT_FRAME)})
            else:
        ...
        for sheet in Sheet.objects.filter(program=self.program.id, is_proto=True, deleted=False):
            ...
            for f in fields:
                ...
                if f.type.name in ['ManyToManyField', 'DictArray', 'RepM2MField']:
                    ...
                    try:
                        to_sheet = Sheet.objects.get(
                            program=self.program.id, name=f.to, deleted=False)
                    except Sheet.DoesNotExist:
                        public.print_str(
                            'ManyToManyField sheet [%s] does not exist' % f.to)
                    else:
                    ...
                    for to in json.loads(f.to):
                        try:
                            to_sheet = Sheet.objects.get(
                                program=self.program.id, name=to, deleted=False)
                        except Sheet.DoesNotExist:
                            public.print_str(
                                'ManyForeignsField sheet [%s] does not exist' % to)
                            errors.append(
                                err1 + '多外键字段[%s(%s)]不存在对应外键表[%s]' % (f.intro, f.name, to))
                        else:
...
#                            errors.append(err1+'记录[%s]外键字段[%s(%s)]中不存在[%s]'%(record.id,f.intro,name,record.__getattribute__(f.name)))
                        ...
                        if v2.deleted:
                            record2 = model_to_dict(record)
                            errors.append(
                                err1 + '记录[%s]外键字段[%s(%s)]中不存在[%s]' % (record.id, f.intro, name, record2.get(f.name, '')))
#                            errors.append(err1+'记录[%s]外键字段[%s(%s)]中不存在[%s]'%(record.id,f.intro,name,record.__getattribute__(f.name)))
                        elif f.to_field:
                            ...
                            if isinstance(v2, unicode) and v2.startswith('[') and v2.endswith(']'):
                                to_sheet = Sheet.objects.get(
                                    program=self.program.id, name=f.to, deleted=False)
                                to_field2 = Field2.objects.get(
                                    model=to_sheet.id, name=f.to_field)
                                if to_field2.type.name == 'DictArray':
                                    try:
                                        to_sheet3 = Sheet.objects.get(
                                            program=self.program.id, name=to_field2.to, deleted=False)
                                    except Sheet.DoesNotExist:
                                        errors.append(
                                            '外键中的字典数组找不到数据:' + f.intro + f.name + '  ' + f.to + '  ' + f.to_field + ' %s' % to_field2.to)
                                        v2 = []
                                    else:
                                        ...
                                        for _id in json.loads(v2):
                                            try:
                                                v4 = to_obj.objects.get(
                                                    id=_id, deleted=False)
                                            except to_obj.DoesNotExist:
                                                errors.append(
                                                    err1 + '记录[%s]字典数组字段[%s(%s)]中不存在[%s]' % (record.id, f.intro, name, _id))
                                            else:
                                                v3.append(
                                                    model_to_dict(v4, exclude=BASE_AUTO_ADD_FIELD))
                                        v2 = v3
                        else:
                            v2 = getattr(v2, 'sn', 0)
                        ...
                        if to_objs.has_key(name):
                            ...
                            for _id in json.loads(v2):
                                try:
                                    ...
                                    # v3.append(_id)
                                    errors.append(
                                        err1 + '记录[%s]多对多字段[%s(%s)]中不存在[%s]' % (record.id, f.intro, name, _id))
                                else:
                                    if f.to_field:
                                ...
                                try:
                                    ...
                                    for f2 in Field2.objects.filter(model=to_sheet.id):
                                        if f2.type.name == 'ForeignKey':
                                            ...
                                            if not to_field:
                                                to_field = 'sn'
                                            up[f2.name] = v4.__getattribute__(
                                                f2.name).__getattribute__(to_field)
                                    v5 = model_to_dict(
                                        v4, exclude=BASE_AUTO_ADD_FIELD)
                                    v5.update(up)
                                    v3.append(v5)
                            v2 = v3
                        ...
                        if to_objs.has_key(name):
                            ...
                            if v3 is None:
                                errors.append(
                                    err1 + '记录[%s]多外键字段[%s(%s)]中不存在[%s]' % (record.id, f.intro, name, v2))
                    v1[name] = v2
        ...
        for record in dyn_obj.objects.filter(deleted=False):
            ...
            for f in fields:
                ...
                if v2 is None:
                    ...
                    if v2.deleted:
                        errors.append(
                            err1 + '记录[%s]外键字段[%s(%s)]中不存在[%s]' % (record.id, f.intro, name, record.__getattribute__(f.name)))
                    elif f.to_field:
                        v2 = getattr(v2, f.to_field, 0)
                    else:
                        v2 = getattr(v2, 'sn', 0)
        ...
        if not table:
            ...
            try:
                ...
                for src_table_name in json.loads(sheet_obj.formula.tables):
                    #                    src[src_table_name] = []
                    src_table_obj = Sheet.objects.get(
                        name=src_table_name)  # .get_django_model()
...
#                        break
    ...
    def charting(self):
        ...
        if self.reqmode != 'js':
            self.content['formulas'] = Formula.objects.filter(
                deleted=False, ft=2)
        else:
            ret = 1
            msg = ""
...
#                               'y': 'Popularity'}
                        ...
                        xpoints = []
                        datas = {
                            'td': {'cols': [{'id': 'id_x', 'label': 'x', 'type': 'number'}], 'rows': []}}
                        for name, v in dst.items():
                            if name in ['x', 'y']:
                                datas[name] = dst.pop(name)
                            else:
                                xpoints.extend(v.keys())
                                datas['td']['cols'].append(
                                    {'id': 'id_' + name, 'label': name, 'type': 'number'})
        ...
        if self.request.method == 'POST':
            ...
            if not fhandler:
                ...
                for table, datas in tables.items():
                    ...
                    print table
                    # 查找表
                    table_obj = sql.find_one(
                        'sheet', fields=['id'], kwargs={'name': table})
                    if not table_obj:
                    ...
                    for data in datas[0]:
                        ...
                        if data .has_key('sn'):
                            ...
                            if len(sn) != sn_rule_len or not sn.startswith('%s' % table_id):
                                ...
                                exist_sns.append(sn)
                    if error_sns:
                        self.tips += '错误SN规则错误，请参考策划后台经行修改:' + \
                            ','.join(error_sns) + '<br/>'
                    if exist_sns:
                        self.tips += '已存在以下SN记录，请仔细检查excel文件:' + \
                            ','.join(exist_sns) + '<br/>'
                    else:
        ...
        try:
            ...
            if procid < 1:
                popen = subprocess.Popen(
                    shlex.split(cmd), shell=True, stdout=None, stderr=None)
                if psutil.pid_exists(procid):
                    print '----------- procid1 is exist: ', procid
                    # time.sleep(5)
    ...
    def sheet_init(self):
        ...
        try:
            if self.sheetid == 'model':
                ...
                self.dyn_model = Sheet.objects.get(
                    program=self.program.id, name=self.sheetid)
#            self.dyn_model  = Sheet.objects.get(program=self.program.id,name=self.sheetid)
            ...
            if self.sheetid == 'model':
                ...
                self.content['tables'] = Sheet.objects.filter(
                    program=self.program.id)
                self.content['sys_models'] = SystemModels.objects.filter(
                    program=self.program.id)
                self.content['sys_models2'] = SystemModels2.objects.filter(
                    program=self.program.id)
                self.content['formulas'] = Formula.objects.filter(
                    deleted=False, ft=1)
                self.achieves['program'] = self.program
                self.achieves['locale'] = self.locale
                self.pkeys = ['sys_model', 'sys_model2', 'name', 'intro', 'num',
                              'is_proto', 'display', 'field', 'formula', 'is_template']
                self.pvalues = [u'一级目录', u'二级目录', u'英文名', u'中文名', u'最后一个记录ID',
                                u'是否上下行表', u'外键表显示字段(英文名)', u'字段', u'公式', u'是否模板']
            elif self.sheetid == 'formula':
                self.content['tables'] = Sheet.objects.filter(
                    program=self.program.id)
...
#                print 'self.dyn_model.name:',self.dyn_model.name
                self.fields = Field2.objects.filter(
                    model=self.dyn_model.id).exclude(name__in=BASE_AUTO_ADD_FIELD)
                foreigns = {}
                for f in self.fields:
                    ...
                    if f.type.name == 'ForeignKey':
                        self.foreign_keys[f.name] = Sheet.objects.get(
                            program=self.program.id, name=f.to).display
                        ...
                        to_obj = Sheet.objects.get(
                            program=self.program.id, name=f.to)
                        ...
                        for f2 in to_obj.fields.exclude(name__in=BASE_AUTO_ADD_FIELD):
                            if f2.name != display:
                                primary_key = f2.name
                                break
                        self.foreign_keys[f.name] = {
                            'to_obj': to_obj.get_django_model(), 'display': display, 'primary_key': primary_key}
                        ...
                        self.foreign_keys[f.name] = {
                            'to_obj': [], 'display': 'name', 'primary_key': 'id'}
                        to_objs = []
                        for to in json.loads(f.to):
                            to_obj = Sheet.objects.get(
                                program=self.program.id, name=to)
...
#                    print 'foreigns',foreigns
                ...
                if self.dyn_model.is_template:
                    ...
                    self.content['filters'] = json.loads(
                        self.dyn_model.filters)
...
#                    v1  = []
                    if public.objIsFull(record_count):
                        ...
                        for i in range(1, int(record_count)):
                            field_id = self.string2other(
                                self.request.POST.get(k + '_id%s' % i, 0), 'int', name + '记录ID')
                            status_id = self.string2other(
                                self.request.POST.get(k + '_status%s' % i, 1), 'int', name + '记录状态')
#                            if field_id is None:
#                                continue
                            ...
                            to_field = self.request.POST.get(
                                k + '_to_field%s' % i)
                            max_length = self.request.POST.get(
                                k + '_max_length%s' % i)
#                            help= self.request.POST.get(k+'_help%s'%i)
                            is_condition = self.request.POST.get(
                                k + '_is_cond%s' % i)
                            if is_condition == "1":
                                conds.append(name1)
#                            default = self.request.POST.get(k+'_default%s'%i)
                            if public.objsIsEmpty([name1, intro, type1]):
                                ...
                                public.print_str(
                                    'name,intro,type1: %s, %s, %s' % (name1, intro, type1))
                                # break
                            else:
                                #                                sets= []
                                try:
                                    type_obj = FieldType2.objects.get(
                                        id=int(type1))
...
#                                    to  =
                                if type_obj is not None and type_obj.name in ['CharField', 'ImageField', 'JSONChar']:
                                    if public.objIsEmpty(max_length):
                                        max_length = 255
                                    max_length = self.string2other(
                                        max_length, 'int', name + '最大长度')
                                else:
                                    max_length = 0
                                if type_obj.name == 'ManyForeignsField':
                                    to_field = 'sn'
                                    to = self.request.REQUEST.getlist(
                                        k + '_to%s' % i)
                                    if not to:
                    ...
                    if k in ['num', 'sys_model', 'sys_model2', 'formula']:
                        ...
                        if k == 'is_template':
                            filters = {}
                            if v1:
                                filters['inst_name'] = public.req_param_rstrip(
                                    self.request.POST.get('inst_name', ''))
                                filters['inst_field'] = public.req_param_rstrip(
                                    self.request.POST.get('inst_field', ''))
            ...
            if public.objIsEmpty(self.tips) and self.do_what == 'add':
                if Sheet.objects.filter(program=self.program.id, name=self.achieves.get('name')):
                    ...
                    self.fields.extend([{'name': 'id', 'intro': 'ID', 'type': FieldType2.objects.get(name='AutoField'), 'to': '', 'max_length': 0, 'id': 0, 'status': 1},
                                        {'name': 'createtime', 'intro': '创建时间', 'type': FieldType2.objects.get(
                                            name='DateTimeField'), 'to': '', 'max_length': 0, 'id': 0, 'status': 1},
                                        {'name': 'username', 'intro': '操作用户', 'type': FieldType2.objects.get(
                                            name='CharField'), 'to': '', 'max_length': 50, 'id': 0, 'status': 1},
                                        {'name': 'deleted', 'intro': '删除', 'type': FieldType2.objects.get(
                                            name='BooleanField'), 'to': '', 'max_length': 0, 'id': 0, 'status': 1},
                                        ])
                    if self.achieves['is_template']:
                        self.fields.append({'name': 'odds', 'intro': '掉落', 'type': FieldType2.objects.get(
                            name='OddsField'), 'to': '', 'max_length': 0, 'id': 0, 'status': 1})
#                    print "sheet_do_add_req_data:", self.fields
            ...
            for index in range(0, len(self.pkeys)):
                ...
                name = self.pvalues[index]
                if k == 'tables':
                    v1 = json.dumps(public.req_param_rstrip(
                        self.request.POST.get(k + '_selected')).strip(',').split(','))
                else:
                    v1 = public.req_param_rstrip(self.request.POST.get(k))
                    if k == 'code':
                        v1 = v1.strip('\r\n')
            ...
            for f in self.fields:
                ...
                if f.type.name in ['ManyToManyField', 'DictArray', ]:
                    if self.reqmode == 'js':
                        ...
                        v1 = self.request.POST.get(
                            '%s_selected' % f.name).strip(',')
                        if v1:
                    ...
                    if self.reqmode == 'js':
                        ...
                        if public.objIsEmpty(path_len):
                            ...
                            for i in range(1, int(path_len)):
                                prefix = '第%s行' % i
                                v2 = public.req_param_rstrip(
                                    self.request.POST.get(f.name + '%s' % i))
                                if public.objIsFull(v2):
                                    v1.append(
                                        self.string2other(v2, 'int', prefix + f.intro))
                            v1 = json.dumps(v1)
                elif f.type.name == 'ManyForeignsField':
                    v1 = 0
                    for k1 in json.loads(f.to):
                        v1 = int(
                            self.request.POST.get('%s_%s' % (f.name, k1), '0'))
                        if DEBUG:
                            print '------- k, v: ', k1, v1
                        if v1 > 0.1:
                            break
                    ...
                    if self.reqmode == 'js':
                        ...
                        if public.objIsEmpty(path_len):
                            ...
                            for i in range(1, int(path_len)):
                                prefix = '第%s行' % i
                                rare = public.req_param_rstrip(
                                    self.request.POST.get(f.name + '_rare%s' % i, ''))
                                odds = public.req_param_rstrip(
                                    self.request.POST.get(f.name + '_odds%s' % i, '0'))
                                cards = [
                                    int(r) for r in self.request.POST.getlist(f.name + '_card%s' % i)]
                                v1.append({'rare': rare,
...
#                    print f.name,v1, self.do_what
                    ...
                    if type_name == 'IntegerField':
                        ...
                        try:
                            ...
                            try:
                                ...
                                self.tips = f.intro + \
                                    '，表[%s]中不存在记录[%s]' % (self.sheetname, v1)
                                v1 = None
                    elif type_name == 'BooleanField':
                        v1 = bool(v1 == '1')
                if f.name == 'name':
        ...
        if self.sheetid == 'model':
            ...
            for f in self.fields:
                if f['name'] in BASE_AUTO_ADD_FIELD:
                    del f
                i += 1
            # json.loads(self.achieves['field'])
            self.achieves['field'] = self.fields
            ...
            for f in self.achieves['field']:
                ...
                if f['type'].name == 'ForeignKey':
                    if DEBUG:
                        print 'name: ', f['name']
                    f['fields'] = [f2.name for f2 in Sheet.objects.get(
                        name=f['to']).fields.all().exclude(name__in=BASE_AUTO_ADD_FIELD)]
                elif f['type'].name == 'ManyForeignsField':
                    if DEBUG:
                        print 'name: ', f['name']
    ...
    def do_modify_get_special_field(self, db_obj):
        ...
        if self.sheetid == 'model':
            ...
            db_obj['field'] = Field2.objects.filter(
                model=model_id).exclude(name__in=BASE_AUTO_ADD_FIELD)
...
#                i   += 1
        ...
        self.content['sheets'] = get_sheets(
            program=self.program, locale=self.locale)
...
#            new_foreign_keys    = []
            ...
            for f1 in self.fields:
                ...
                if field_id == 0:
                    ...
                    f_obj.save()
                    add_db_table_column(
                        django_model, f['name'], f_obj.get_django_field())
                else:
                    if status == 1:
                        new_fields2.append(f['name'])
                        continue
                        ...
                        try:
                            ...
                            public.print_str(
                                'wariming: field id [%s] is not exists' % field_id)
                        else:
                            if public.objIsEmpty(f_objs):
                                public.print_str(
                                    u'记录ID【%s】不存在，请检查数据的正确性' % self.recordid)
                            else:
                                if status == 2:
                                    # 删除
                                    # 原字段名
    ...
    def do_achieve_recordid(self):
        ...
        if self.do_what in ['modify', 'delete'] and self.sheetid in ['topo_graph'] and self.reqmode == 'js':
            if self.do_what == 'modify':
                ...
                self.recordid = json.loads(
                    self.request.raw_post_data).get('id', '0')
            else:
...
#            self.recordid   = int(self.recordid) + 1000
    ...
    def do_achieve_records(self, page, id=0):
        ...
        if self.reqmode == 'js':
            if self.recordid:
                paginator, records = self.paging(
                    self.sheet_obj, 10000, 1, deleted=False, id=self.recordid)
            else:
                paginator, records = self.paging(
...
#                self.pvalues.append()
    ...
    def do_spec_table(self):
        ...
        if self.sheetid == 'trade_npc':
            ...
            for record in records:
                datas.append(model_to_dict(record, fields=self.pkeys))
            public.write_json_file(
                PARAM_DEBUG_PATH, {'trad_npc': datas}, end='\n')
                ...
                for f in self.fields:
                    ...
                    if not v1:
                        ...
                        if f.type.name == 'ImageField':
                            ...
                            if len(v1) > 8:
                                self.temp_html += '<a href="#" title="%(value)s">%(value_short)s</a>' % {
                                    'value': v1, 'value_short': v1[:8]}
                            else:
                                self.temp_html += v1
                        elif f.type.name == 'ForeignKey':
                            if v1:
                                self.temp_html += '%s' % getattr(
                                    v1, self.foreign_keys[f.name])
                            else:
                                self.temp_html += ''
                        # 于2014-07-01 15:36修改，显示对应的字段列表
                            ...
                            tmp_str = '[' + ', '.join(v2) + ']'
                            if len(tmp_str) > 8:
                                self.temp_html += '<a href="#" title="%s">%s...</a>' % (
                                    tmp_str, tmp_str[:8])
                            else:
    ...
    def translate_to_html(self, db_obj, fields, dyn_model):
        ...
        for f in fields:
            ...
            if f.type.name == 'BooleanField':
                ...
                if v1:
                    html += '<input id="id_%(table)_%(field)s" name="%(field)s" value="1" type="checkbox" checked="checked" />' % {
                        'field': f.type.name, 'table': table}
                else:
                    html += '<input id="id_%(table)s_%(field)s" name="%(field)s" value="1" type="checkbox" />' % {
                        'field': f.name, 'table': table}
                    ...
                    if f.type.name == 'ForeignKey':
                        ...
                        for record in model_obj.objects.filter(deleted=False):
                            param1 = {'id': record.id, 'display': getattr(
                                record, to_obj.display), 'table': table}
                        ...
                        for d1 in v1:
                            ...
                            for record in model_obj.objects.filter(deleted=False):
                                param2 = {
                                    'id': record.id, 'display': getattr(record, to_obj.display)}
                ...
                for to_obj, model_obj in self.foreign_keys[f.name]['to_obj']:
                    ...
                    for record in model_obj.objects.filter(deleted=False):
                        ...
                        param1 = {
                            'sn': sn, 'display': getattr(record, 'name', _id)}
                        if v1 and sn == v1:
                ...
                html += '<input id="id_%(table)s_%(field)s" name="%(field)s" style="width:150px;" value="%(value)s" type="text" />' % {
                ...
                html += '&nbsp;&nbsp;<a href="javascript:void();" onclick="javascript:check_json(\'id_%(field)s\',\'%(intro)s\');">检查JSON格式</a>' % {
                    'field': f.name, 'intro': f.intro}
                self.content['js_str'] = "javascript:return check_json(\'id_%(field)s\',\'%(intro)s\',\', 确认提交\');" % {
                    'field': f.name, 'intro': f.intro}
            else:
                html += '<input id="id_%(table)s_%(field)s" name="%(field)s" style="width:150px;" value="%(value)s" type="text" />' % {
                    'field': f.name, 'value': v1, 'table': table}
    ...
    def edit_template(self, db_obj):
        ...
        for f in self.fields:
            ...
            if f.type.name == 'BooleanField':
                ...
                if v1:
                    self.temp_html += '<input id="id_%(field)s" name="%(field)s" value="1" type="checkbox" checked="checked" />' % {
                        'field': f.type.name}
                else:
                    self.temp_html += '<input id="id_%(field)s" name="%(field)s" value="1" type="checkbox" />' % {
                        'field': f.name}
                ...
                try:
                    ...
                    model_obj = to_obj.get_django_model()
                    if f.type.name == 'ForeignKey':
                        self.temp_html += '<select id="id_%(name)s" name="%(name)s" style="width:150px;height:20px;">' % {
                            'name': f.name}
                        ...
                        try:
                            ...
                            if DEBUG:
                                print 'v1: ', v1
                            v1 = []
                        param1 = {'name': f.name, 'to': f.to, 'table': self.sheetid, 'value': ','.join(
                            [str(v2) for v2 in v1])}
                        ...
                        if f.type.name == 'DictArray':
                            ...
                            self.temp_html += '<button type="button" class="btn btn-primary" data-toggle="modal" data-target=".%s-modal-lg">添加%s</button>' % (
                                f.to, to_obj.intro)
                            self.temp_html += '</div>'
                            self.temp_html += '</div>'
                            self.temp_html2 += '<div class="modal fade %(to)s-modal-lg" tabindex="-1" role="dialog" aria-labelledby="%(to)s_largeModalLabel" aria-hidden="true" id="id_%(to)s_modal">' % {
                                'to': f.to}
                            ...
                            mm_fields = Field2.objects.filter(
                                model=to_obj.id).exclude(name__in=BASE_AUTO_ADD_FIELD)
                            self.temp_html2 += self.translate_to_html(
                                None, mm_fields, to_obj)
                            #self.temp_html2 += '</table>'
                            ...
                            self.temp_html2 += '''<button type="button" class="btn btn-primary" onclick="javascript:save_record('%s','%s', '%s', '%s')">保存</button>''' % \
                                (f.to, ','.join(
                                    [f2.name for f2 in mm_fields]), to_obj.display, 'id_' + self.sheetid + '_' + f.name)
                            self.temp_html2 += '</div>'
                            self.temp_html2 += '</div>'
                            self.temp_html2 += '</div>'
                            self.temp_html2 += '</div>'
                ...
                for to_obj, model_obj in self.foreign_keys[f.name]['to_obj']:
                    ...
                    self.temp_html  += '<p><span style="width:100px;">%(to_intro)s：</span><select id="id_%(name)s_%(to_name)s" name="%(name)s_%(to_name)s" style="width:250px;height:20px;">' % \
                        {'name': f.name, 'intro': f.intro,
                            'to_name': to_obj.name, 'to_intro': to_obj.intro}
                    self.temp_html += '<option value="0">请选择一个%s</option>' % to_obj.intro
                    for record in model_obj.objects.filter(deleted=False):
                        _id = record.id
                        sn = getattr(record, 'sn', 0)
                ...
                self.temp_html += '<input id="id_%(field)s" name="%(field)s" style="width:150px;" value="%(value)s" type="text" />' % {
                    'field': f.name, 'value': v1}
                ...
                if self.dyn_model.filters:
                    ...
                    if inst_name:
                        inst_sheet = Sheet.objects.get(
                            program=self.program.id, name=inst_name, deleted=False)
                        inst_sheet_obj = inst_sheet.get_django_model()
                        if inst_field:
                            inst_field = Field2.objects.get(
                                model=inst_sheet.id, name=inst_field)
                # 写表头
                param1 = {
                    'name': f.name, 'index': 0, 'inst_field_name': inst_field.name}
                self.temp_html += '<p>'
                ...
                if self.do_what == 'modify':
                    ...
                    if v1:
                        for rare, cards in frecords.items():
                            ...
                            if inst_field:
                                self.temp_html += '<td>'
                                self.temp_html += '<input id="id_%(name)s_rare%(index)s" name="%(name)s_rare%(index)s" style="width:100px;" ' % param1 + \
                                    'value="%s" readonly="readonly" >' % rare
                                self.temp_html += '</td>'
                            ...
                            self.temp_html += '</td>'
                            self.temp_html += '<td>'
                            self.temp_html += '<input id="id_%(name)s_odds%(index)s" name="%(name)s_odds%(index)s" style="width:100px;" ' % param1 + \
                                'value="%s" type="text" />' % odds
                            self.temp_html += '</td>'
                            # 记录筛选
                            self.temp_html += '<td>'
                            for r in cards:
                                ...
                                if r.sn in checked_cards:
                                    self.temp_html += '<input type="checkbox" checked="checked" value="%s"' % r.sn + \
                                        ' name="%(name)s_card%(index)s" id="id_%(name)s_card%(index)s_' % param1 + \
                                        '%s">' % r.sn
                                else:
                                    self.temp_html += '<input type="checkbox" value="%s"' % r.sn + \
                                        ' name="%(name)s_card%(index)s" id="id_%(name)s_card%(index)s_' % param1 + \
                                        '%s">' % r.sn
                                self.temp_html += '<span style="color:#000;">&nbsp;%s</span>' % getattr(
                                    r, display)
                                self.temp_html += '</label>'
                            self.temp_html += '</td>'
                            self.temp_html += '</tr>'
                ...
                self.temp_html += '<textarea id="id_%(field)s" name="%(field)s" style="width:350px;height:120px;">%(value)s</textarea>' % {
                    'field': f.name, 'value': v1}
            elif f.type.name == 'JSONChar':
                #                print f.name, v1
                self.temp_html += '<input id="id_%(field)s" name="%(field)s" style="width:350px;" value=\'%(value)s\' type="text" />' % {
                    'field': f.name, 'value': v1}
                self.temp_html += '&nbsp;&nbsp;<a href="javascript:void();" onclick="javascript:check_json(\'id_%(field)s\',\'%(intro)s\');">检查JSON格式</a>' % {
                    'field': f.name, 'intro': f.intro}
                self.content['js_str'] = "javascript:return check_json(\'id_%(field)s\',\'%(intro)s\',\', 确认提交\');" % {
                    'field': f.name, 'intro': f.intro}
            else:
            ...
            if f.type.name == 'BooleanField':
                ...
                try:
                    ...
                    if f.type.name == 'ForeignKey':
                        ...
                        self.temp_html += '<button type="button" class="btn btn-primary" data-toggle="modal" data-target=".%s-modal-lg">添加%s</button>' % (
                            f.to, to_obj.intro)
                        self.temp_html += '<div class="modal fade %(to)s-modal-lg" tabindex="-1" role="dialog" aria-labelledby="%(to)s_largeModalLabel" aria-hidden="true" id="id_%(to)s_modal">' % {
                            'to': f.to}
                        ...
                        mm_fields = Field2.objects.filter(
                            model=to_obj.id).exclude(name__in=BASE_AUTO_ADD_FIELD)
                        self.temp_html += self.translate_to_html(
                            None, mm_fields, to_obj)
                        self.temp_html += '</table>'
                        self.temp_html += '</form>'
                        self.temp_html += '</div>'
            ...
            for f in self.fields:
                ...
                if not v1:
                    ...
                    if f.type.name == 'ForeignKey':
                        if v1:
                            rd2.append(
                                '%s' % getattr(v1, self.foreign_keys[f.name]))
                        else:
                            rd2.append('')
                    elif f.type.name in ['ManyToManyField', 'DictArray', 'RepM2MField']:
                        v2 = []
                    ...
                    # 修改记录
                        ...
                        if public.objIsEmpty(self.recordid):
                            ...
                            try:
                                ...
                                db_objs = self.sheet_obj.objects.filter(
                                    **self.filter_kwargs)
                            # except self.sheet_obj.DoesNotExist:
                            except:
                                ...
                                if public.objIsEmpty(db_objs):
                                    ...
                                    public.print_str(
                                        u'记录ID【%s】更新， %s' % (self.recordid, self.achieves))
