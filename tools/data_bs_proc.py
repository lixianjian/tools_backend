...
#                    continue
    ...
    def write_ex_sell(self):
        ...
        while True:
            if start_date > end_date:
                break
            msg,records = public.get_log_records(bpath=self.server_info['gl_path'],start_day=start_date,end_day=end_date,run_hour=False,include=['ExSell'])
            if msg:
                print 'error:',msg
            else:
...
#            刷新玩家拓扑结构RefreshPlayerTopo
            msg,records = public.get_log_records(bpath=self.server_info['gl_path'],start_day=start_date,end_day=end_date,run_hour=False,
...
def do_plat_pay_info(sid,start_day,end_day):
    ...
    tongbao = 0
    obj = DataBackstage(sid, create=False)
    obj.set_date(p_start_date=start_day,p_end_date=end_day)
    records = obj.get_pay_record(ps_date=obj.start_date, pe_date=obj.end_date, roleids=None)
    for record in records:
        #tb  = record.get('tongbao',0)
        #print sid,'\t',tb
...
def get_plat_pay_info(start_day,end_day):
    ...
    for server in SERVERS:
        ...
        if datas.has_key(plat_id):
            ...
            datas[plat_id]  = {'rmb':rmb,'platname':PLAT_NAMES.get(plat_id,plat_id)}
    return datas.values()
...
#            if start_date < '2013-03-11':
#                continue
#            sname= server['name']
#            is_cross= server.get('is_cross',False)
            obj = DataBackstage(sid, create=False)
            obj.set_server_info()
            if not obj.server_info:
                continue
            obj.set_date(p_start_date=None,p_end_date=None)
            #写卖出货物获利
            obj.write_ex_sell()
            #写行动力消耗
            obj.write_cost_ap()
...
if __name__ == "__main__":
    #os.rename('D:/check_data/db/sea/username_ip.txt.tmp', 'D:/check_data/db/sea/username_ip.txt')
    #执行正式模块
    EXCUTE_PROCESS  = True
    #执行测试模块
    EXCUTE_TEST     = False
    if EXCUTE_PROCESS:
        ...
        obj     = DataBackstage('1', create=False, start_day='2012-12-09', end_day='2012-12-09')
